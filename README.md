...
Styp (Sum TYPes) in Javascript. The library provides `tagged` and `tagged sum` types, the distribution of the library is `611` bytes. This libary has been inspired by languages like `Haskell`, `F#` and `OCaml` which provide sum types natively and also has taken inspiration from [daggy]() (which is also a library for creating sum types in Javascript).
